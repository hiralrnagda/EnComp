...
def encode(filename, directory):  # compress the file
    ...
    #start = time.time()
    #print(filename)
    input_file = open("EncryptedFiles/"+ filename,"rb")  # opens the file from the InputFiles directory
    input_size = os.path.getsize("EncryptedFiles/" + filename)  # finds the input size for later comparison
    ...
    output_file = open("CompressedFiles/" + filename.split(".")[0] + ".txt", "wb")
...
def decode(filename, output_extension):
    ...
    input_file = open("CompressedFiles/" + filename.split(".")[0] + ".txt","rb")  # reads the file in binary
    ...
    text = bitarray(code)
    output_file.write(''.join(text.decode(code_book_dict)))  # decodes the text from code_book and writes it to file as a string
    output_file.close()
    output_size = os.path.getsize("DecompressedFiles/" + filename.split(".")[0] + output_extension)
    print("Decompressed Size --->" + str(output_size) + " bits\n")
...
def conduct_test(filename):
    ...
    encode(filename, "")
    print("Compression Time : %s seconds\n\n" % (time.time() - start_time))
    
    #print("\n Decompressing : \n")
    start_time = time.time()
    decode(filename, ".txt")
    print("Decompression Time : %s seconds\n" % (time.time() - start_time))
    #open("InputFiles/" + filename.split(".")[0] + ".txt", 'rb').read() == open("DecompressedFiles/" + filename.split(".")[0] + ".txt", 'rb').read()
